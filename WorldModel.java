...
public class WorldModel
{
   ...
   public void addJames (James james)
   {
      Point pt = james.getPosition();
      for (int dy = 0; dy <3 ; dy++)
      {
         for (int dx = 0; dx <3; dx++)
         {
            setCell(occupancy, new Point (pt.x +dx, pt.y + dy), james);
         }
      }
      entities.add(james);
   }

   public void addJesse (Jesse jesse)
   {
      Point pt = jesse.getPosition();
      for (int dy = 0; dy <3 ; dy++)
      {
         for (int dx = 0; dx <3; dx++)
         {
            setCell(occupancy, new Point (pt.x + dx, pt.y + dy), jesse);
         }
      }
      entities.add(jesse);
   }

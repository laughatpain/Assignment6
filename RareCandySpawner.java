   ...
   extends Actor
{
   ...
   public RareCandySpawner(String name, Point position, int rate, int resourceDistance,
                           List<PImage> imgs)
   {
   ...
   public Action createAction(WorldModel world, ImageStore imageStore)
   {
      ...
      action[0] = ticks -> {
         ...
         if (openPt != null)
         {
            RareCandy ore = createOre(world, "ore - " + getName() + " - " + ticks,
   ...
   private RareCandy createOre(WorldModel world, String name, Point pt,
      long ticks, ImageStore imageStore)
   {
      RareCandy ore = new RareCandy(name, pt,
